...
function ModuleMenu({mod,groupCount=1,onClose,onUpdateMod,onUpdateModLive,onRemove,onDuplicate,emptyCount=1,onFillHardpoints,fillCount=0,resourceHeadroom,engineItem,chargeStats}){
    ...
    <BottomSheet title={mod.name} onClose={onClose} height="78vh">
      ...
      <div onScroll={dismissKeyboardOnScroll} style={{padding:14,overflowY:'auto',maxHeight:'60vh'}}>
        ...
        {_isRAH&&(()=>{
          ...
          {/* One empty slot left is not a choice, so it stays the single button it has always been.
              From two up it becomes a row of count chips: a rack never has more than eight slots, so
              every count fits on screen and any of them is one tap — a stepper would cost a tap per
              increment before the fill even starts. The ceiling is the section's EMPTY SLOTS, not
              what the fit can power: putting a module into a slot the hull has is always allowed and
              going over CPU/PG shows red afterwards, exactly as it does when the modules are added
              one at a time. */}
          {onDuplicate&&(emptyCount>1?(<div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:C.textMute,marginBottom:7}}>{t("Duplicate to Empty Slots")}</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {Array.from({length:emptyCount},(_,i)=>i+1).map(n=>(
                <button key={n} className="press" onClick={()=>{haptic("medium");onDuplicate(n);onClose();}} style={{flex:"1 1 0",minWidth:54,padding:"11px 0",background:C.accentLight,border:`1px solid ${C.accentBorder}`,borderRadius:8,color:C.accent,fontSize:13,fontWeight:700,cursor:"pointer"}}>{n===emptyCount?t("All {n}",{n}):`×${n}`}</button>))}
            </div>
          </div>):(
