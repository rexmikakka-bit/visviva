...
export function calcFitStats(ship, slots, drones = [], skills = SKILL_DEFAULTS, opts = {}) {
  ...
  for (const { slot, fitItem } of modItems) {
    ...
    const isAAR = modName.includes('Ancillary Armor Repairer') ||
    ...
    if (isAAR) {
      ...
      // Paste phase time only (Pyfa style: no reload)
      const pastePhaseS = cycleMs * PASTE_CYCLES / 1000;
